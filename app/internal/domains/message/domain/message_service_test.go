...
func TestHealthCheck_Timeout(t *testing.T) {
	...
	// Set up a context that will time out quickly
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Millisecond)
	defer cancel()

	// Storage service will take longer than the context timeout
	stor.On("CheckHealth", mock.Anything).Run(func(args mock.Arguments) {
		time.Sleep(50 * time.Millisecond)
	}).Return("healthy", nil)

	enc.On("CheckHealth", mock.Anything).Return("healthy", nil)
	notif.On("CheckHealth", mock.Anything).Return("healthy", nil)

	// Call HealthCheck. It should return due to context timeout.
	resp, err := svc.HealthCheck(ctx)

	assert.NoError(t, err)
	assert.NotNil(t, resp)
	assert.Equal(t, "unhealthy", resp.Status)
	assert.Equal(t, "timeout", resp.Services["database"])

	// Small sleep to ensure the slow goroutine finishes
